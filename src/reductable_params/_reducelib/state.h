...
typedef struct {
    PyObject* _abc_Reductable; /* .abc.Reductable */
    PyObject* _utils_varnames; /* .utils.varnames */
    PyObject* __name__; /* "__name__" */
    PyObject* function; /* "function" */
    PyTypeObject* ReduceType;
} mod_state;
...
/* Borrowing multidict's code until this part is ulitmately dropped by multidict itself
* I also maintain that library too :) */
...
get_mod_state_by_def_checked(PyObject *self, mod_state **ret)
{
    ...
    PyObject *mod = PyType_GetModuleByDef(tp, &reduce_module);
    if (mod == NULL) {
        *ret = NULL;
